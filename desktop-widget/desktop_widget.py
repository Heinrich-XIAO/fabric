...
PYWAL = False
PROFILE_PICTURE = os.path.expanduser("~/Pictures/Other/profile.png")
...
class SidePanel(Window):
    ...
    def get_current_uptime(self):
        ...
        return f"{int(uptime_days) if uptime_days!=0 else ''} {('days,' if uptime_days != 1 else 'day,') if uptime_days != 0 else ''} {int(uptime_hours) if uptime_hours!=0 else ''} {('hours,' if uptime_hours != 1 else 'hour,') if uptime_hours != 0 else ''}  {int(uptime_minutes)} {'minutes' if uptime_hours != 1 else 'minute'}"
